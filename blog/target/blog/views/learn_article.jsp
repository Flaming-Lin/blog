...
<article>
    ...
    <div class="container">
        <div class="about left">
            <ul>
                <h4 class="atitle"><span><%=article.getPublish().toString().substring(0,10)%></span><%=article.getTitle()%><a>点击次数：<%=article.getTotal_click()%></a></h4>
                <p class="newsnav">您现在的位置是 : <a href="<%=contextPath%>/index">首页</a>&nbsp;>&nbsp;<a href="<%=contextPath%>/learn">学无止境</a>&nbsp;>&nbsp;<a href="<%=contextPath%>/learn/show?id=<%=article.getId()%>"><%=article.getTitle()%></a></p>
            </ul>
            ...
            <div class="nextinfo">
                <% if( null == articleLast ){%>
                <p>上一篇：<a href="<%=contextPath%>/learn">返回列表</a></p>
                <% } else { %>
                <p>上一篇：<a href="<%=contextPath%>/learn/show?id=<%=articleLast.getId()%>"><%=articleLast.getTitle()%></a></p>
                <% } %>
                <% if( null == articleNext ){%>
                <p>下一篇：<a href="<%=contextPath%>/learn">返回列表</a></p>
                <% } else { %>
                <p>下一篇：<a href="<%=contextPath%>/learn/show?id=<%=articleNext.getId()%>"><%=articleNext.getTitle()%></a></p>
                <% } %>
            </div>
            <div class="otherlink">
                <h2>相关文章</h2>
